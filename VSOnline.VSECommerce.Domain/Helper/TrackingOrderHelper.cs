...
//       public static string GetPagingOrders(int PageId)
//       {
//           var currentUser = ClaimsPrincipal.Current.Identity.Name;
//           var query = @" WITH OrdersCte AS
//               (
...
//                    , OrdersProductCte AS
//                  (
//                   select  orders.Id,orders.OrderDateUtc,product.OrderId,product.ProductId,product.BranchId,product.Quantity,product.PriceInclTax,product.OrderItemStatus,orders.CustomerId,orders .OrderTotal,orders.OrderStatusId,orders.PaymentMethod from OrderProduct orders inner join OrderProductItem product on orders .Id=product .OrderId where orders.CustomerId= (select UserId from [User] where Username='{currentUser}')
//                     )
//   select  count(ProductId)'NumberOfProducts',OrderTotal,Id ,OrderDateUtc,PaymentMethod,OrderStatusId from  OrdersProductCte where OrderDateUtc >= Dateadd(Month, Datediff(Month, 0, DATEADD(m, -6, current_timestamp)), 0) group by Id,OrderDateUtc,PaymentMethod,OrderTotal,OrderStatusId order by Id desc
//                      OFFSET (1-1)*2 ROWS
//                FETCH NEXT ({PageId}*2) ROWS ONLY
//                   
//                         
//       ".FormatWith(new { PageId,currentUser });
//           return query;
//       }
    }
