            ...
            try
            {
                using (SqlConnection con = new SqlConnection(conStr))
                {
                    try
                    {
                      ...
                      //  string desc = ds.Tables[0].Rows[0]["ProductDescriptionHtml"].ToString();
                ...
                using (SqlConnection con = new SqlConnection(conStr))
                {
                    try
                    {
                        string[] selectID = publishIds;
                          ...
                          //  string oldQuery = "select Published from Product where ProductId=" + selectID[publish] + " ";
                           // SqlCommand cmd1 = new SqlCommand(oldQuery, con);
                            //SqlDataAdapter sda = new SqlDataAdapter(cmd1);
                            //sda.Fill(ds);
                            con.Open();
                            ...
                            //    AdminMigratedSaveHistory.insertData(Convert.ToInt32(selectID[publish].ToString()), "1", "0", "Product", "Published");
